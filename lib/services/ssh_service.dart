...
class SshService {
  ...
  final bool _isOhos = defaultTargetPlatform == TargetPlatform.ohos;
  ...
  Future<SSHClient> connect(
    ...
    try {
      await _startOhosBackgroundTask();
  ...
  Future<SSHClient> _connectWithTwoFactorAuth(
    ...
    await _updateOhosNotification(connection, '已连接');
  ...
  Future<void> _startOhosBackgroundTask() async {
    if (!_isOhos) return;
    try {
      await platform.invokeMethod('startBackgroundRunning');
    } catch (e) {
      print('启动 OHOS 后台保活失败: $e');
    }
  }

  Future<void> _updateOhosNotification(
    ConnectionInfo connection,
    String status,
  ) async {
    if (!_isOhos) return;
    try {
      await platform.invokeMethod('updateNotification', {
        'title': 'SSH 服务运行中',
        'content': '${connection.name} ${connection.host}:${connection.port} $status',
      });
    } catch (e) {
      print('更新 OHOS 后台通知失败: $e');
    }
  }

  Future<void> _stopOhosBackgroundTask() async {
    if (!_isOhos) return;
    try {
      await platform.invokeMethod('stopBackgroundRunning');
    } catch (e) {
      print('停止 OHOS 后台保活失败: $e');
    }
  }
